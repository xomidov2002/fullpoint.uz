...
const { kameraId } = storeToRefs(store)
const router = useRouter()
const { kameraWorks } = useServiceTable()

function handleClicked(id: any) {
  localStorage.setItem('kameraId', id);
  kameraId.value = id
  router.push({ name: 'one of camera', params: { id: id } })
}
</script>
<template>
  <div
    ...
    <div class="container relative mx-auto px-5 select-none">
      <p
        ...
        {{ t('ourServices.kamera') }}</p>
    </div>
  </div>
  <div class="container mx-auto px-5 py-11">
    <p class="text-white text-3xl font-semibold px-5 border-l-2 py-5 border-l-blue-600 mb-11">{{t('projects.aboutUs')}}</p>
    <p class="text-[#797a7c] mb-10 text-xl">{{ t('projects.kamera') }}</p>
    <p class="text-white text-3xl font-semibold px-5 border-l-2 py-5 border-l-blue-600 mb-11">{{ t('projects.clients') }}</p>
    <!-- <div>
      <table class="">
        <thead>
          <tr>
        ...
        <tbody>
          <tr v-for="(marketingWork, index) in kameraWorks" :key="index">
    ...
    <div class="flex justify-center flex-wrap gap-10 mt-24">
      <BaseCard v-for="(work, index) in kameraWorks" :key="index" :card-data="work" @click-by-id="handleClicked(work.id)"/>
    </div>
  </div>
</template>
<style scoped>
