...
const { moliyaId } = storeToRefs(store)
const router = useRouter()
const { moliyaWorks } = useServiceTable()

function handleClicked(id: any) {
  localStorage.setItem('moliyaId', id);
  moliyaId.value = id
  router.push({ name: 'one of finance', params: { id: id } })
}
</script>
<template>
  <div
  ...
  <div class="container mx-auto px-5 py-11">
    ...
    <p class="text-[#797a7c] mb-10 text-xl">{{ t('projects.moliya') }}</p>
    ...
    <!-- <div>
      <table class="">
        ...
        <tbody>
          <tr v-for="(marketingWork, index) in moliyaWorks" :key="index">
