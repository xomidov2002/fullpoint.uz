...
<template>
  ...
  <div class="container mx-auto px-5 py-11">
    ...
    <p class="mb-10 text-[#797a7c] text-xl">{{ t('projects.it') }}</p>
    <p class="text-3xl font-semibold text-white px-5 border-l-2 py-5 border-l-blue-600 mb-11">{{ t('projects.clients') }}</p>
    <!-- <div>
      <table class="">
        <thead>
          <tr>
        ...
        <tbody>
          <tr v-for="(marketingWork, index) in dasturlashWorks" :key="index">
            ...
            <td>
              <a target="_blank" :href="marketingWork.info" class="cursor-pointer hover:border-b hover:text-[#252B42]">{{t('table.info')}}</a>
            </td>
          </tr>
        </tbody>
      </table>
